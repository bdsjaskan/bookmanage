...
<!--分页-->
    ...
    <el-dialog title="修改密码" :visible.sync="dialogFormVisible" width="30%">
      ...
      <el-form :model="form" label-width="100px" ref="fromRef" :rules="rules">
...
export default {
  ...
  methods:{
    ...
    handleChangePass(row){
      ...
      this.$set(this.form, 'newPass', row.data.newPass)
    ...
    savePass(){
        this.$refs['fromRef'].validate((valid) => {
          if (valid) {
            request.put('/admin/password',this.from).then(res => {
              if (res.code === '200'){
                this.$notify.success("修改成功")
                if (this.from.id === this.admin.id){
                  //当前修改的用户id 等于当前登录的管理 id，那么修改成功之后，那么需要重新登录
    ...
    load(){
      // fetch('http://localhost:80/user/list').then(res => res.json()).then(res => {
      //   console.log(res)
      //   this.tableData = res
      // })
