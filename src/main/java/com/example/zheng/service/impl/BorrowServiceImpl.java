...
import com.example.zheng.mapper.BorrowMapper;
...
public class BorrowServiceImpl implements BorrowService {
    ...
    public void save(Borrow obj) {
        ...
        if (Objects.isNull(book)) {
            throw new ServiceException("所借图书不足");
        }
        //3.校验图书数量
        if (book.getNums() < 1) {
        ...
        Integer score = book.getScore() * obj.getDays(); //借1本的积分 * 天数
        ...
        obj.setReturnDate(LocalDate.now().plus(obj.getDays(), ChronoUnit.DAYS)); //当前日期加 days =归还日期
        obj.setScore(score);
        //7.新增借书记录
    ...
    public void saveRetur(Retur obj) {
        ...
        //提前还书返回用户积分
        Book byNo = bookMapper.getByNo(obj.getBookNo());
        if (byNo != null) {
            long until = 0;
            Integer status;
            if (obj.getRealDate().isBefore(obj.getReturnDate())) { //提前归还
            ...
            int score = (int) (until * byNo.getScore());//获取带归还的积分 (+或许-)
